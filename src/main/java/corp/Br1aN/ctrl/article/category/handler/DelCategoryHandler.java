...
public class DelCategoryHandler implements Handler<RoutingContext> {

  private static final String DEL_CATEGORY = "DELETE FROM public.category WHERE category_id=$1 ";

  private JsonObject dataResponse = null;

  private PgPool pool = null;

  public DelCategoryHandler(PgPool pool){
    this.pool = pool;
  }
  public void handle(RoutingContext context) {
    this.pool.getConnection( ar -> {
      ...
      if (ar.succeeded()) {
        ...
        conn.preparedQuery( DEL_CATEGORY, data, ar2 -> {
          if (ar2.succeeded()) {
